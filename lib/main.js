...
export default {
  ...
  config: {
    ...
    addStar: {
      title: 'Mark by star unclassified flags',
    ...
    allowMagic: {
      ...
      description: 'A ruff by-pass of magic commands e.g. `%timeit`',
      type: 'boolean',
      default: true,
      order: 12,
    },
  ...
  activate() {
    ...
    this.disposables.add(
      ...
      atom.commands.add('atom-workspace', {
        ...
        'linter-ruff:open-default-config': () => {
  ...
  lint(editor, fix=false) {
    return new Promise((resolve, reject) => {
      tmp.file({ postfix:'.py' }, (fileError, tempPath) => {
        ...
        if (this.allowMagic) { editorText = editorText.replace(/^%/gm, '# %') }
